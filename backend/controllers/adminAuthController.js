...
const loginAdminUser = async (req, res) => {
  ...
  try {
    ...
    if (user.company && user.company.status === 'inactive') {
      return res.status(403).json({ error: 'Company is inactive. Cannot log in.' });
    }

    if (user.company && user.company.expiredDate && new Date(user.company.expiredDate) <= new Date()) {
      return res.status(403).json({ error: 'Company subscription has expired. Cannot log in.' });
    }
